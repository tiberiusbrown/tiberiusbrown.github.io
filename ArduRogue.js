...
var ASM_CONSTS = {
  6104: function() {FS.syncfs(function (err) {assert(!err);});},  
 6147: function() {FS.mkdir('/offline'); FS.mount(IDBFS, {}, '/offline'); FS.syncfs(true, function (err) { Module.ccall('initpers', 'v'); });},  
 6270: function($0, $1, $2) {let data = HEAPU8.subarray($0, $0 + $1 * $2 * 4); let canvas = Module['canvas']; canvas.width = window.innerWidth; canvas.height = window.innerHeight; let context = canvas.getContext('2d'); context.imageSmoothingEnabled = false; let temp_canvas = document.createElement('canvas'); let temp_context = temp_canvas.getContext('2d'); temp_canvas.width = 128; temp_canvas.height = 64; let image_data = temp_context.createImageData($1, $2); image_data.data.set(data); temp_context.putImageData(image_data, 0, 0); let scale = Math.floor(Math.min( canvas.width / 128, canvas.height / 64)); let w = 128 * scale; let h = 64 * scale; context.fillStyle = '#000'; context.fillRect(0, 0, canvas.width, canvas.height); context.drawImage( temp_canvas, Math.floor((canvas.width - w) / 2), Math.floor((canvas.height - h) / 2), w, h);}
};
...
/** @type {function(...*):?} */
var _initpers = Module["_initpers"] = createExportWrapper("initpers");
